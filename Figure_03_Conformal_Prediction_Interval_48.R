...
ggplot(data = braz_48, aes(x = Date)) +
  ...
  scale_y_continuous(
    limits = round(c(min(braz_48$Interval_L) - 0.1, max(braz_48$Interval_U) + 0.1),1),
...
ggplot(data = rus_48, aes(x = as.Date(Date))) +
  ...
  scale_x_date(date_breaks = "8 month", date_labels = "%Y-%m-%d", limits = c(ymd("2019-11-01"), ymd("2023-10-01"))) +
  scale_y_continuous(
    limits = round(c(min(rus_48$Observed) - 0.1, max(rus_48$Observed) + 0.1),1),
    breaks = function(limits) seq(limits[1], limits[2], length.out = 5)
  ) +
  ylab('Exchange Rate') +
  xlab('Time') +
...
setwd('Dataset/Dataset_Model_Forecasts/ConfPI')
ind_data <- read.csv("India ConfPI 48.csv")
...
setwd('Dataset/Dataset_Model_Forecasts/ConfPI')
chn_data <- read.csv("China ConfPI 48.csv")
